...
class GameController:
    ...
    def log_game_result(self):
        ...
        first_move = self.board.first_move # record the first square taken
        result = 'Draw' if is_draw else ('Winner:' + winner if winner else 'No Result')
        first_player_result = self.get_first_player_result(first_move, winner, is_draw)

        logging.info(f"{result}, First Move: {first_move}, First Player Result: {first_player_result}") # Update logs
        self.save_to_csv(winner, is_draw, first_move, first_player_result) # save to CSV
    
    def get_first_player_result(self, first_move, winner, is_draw):
        if not first_move:
            return 'No First Move'
        if is_draw:
            return 'Draw'
        return 'Win' if first_move['player'] == winner else 'Loss'
    
    def save_to_csv(self, winner, is_draw, first_move, first_player_result):
        ...
        with open(filename, mode='a', newline='') as file:
            ...
            if not file_exists:
                writer.writerow(['Date', 'Winner', 'Is_Draw', 'First_move', 'First_player_result'])
            
            writer.writerow([datetime.now().strftime('%Y-%m-%d %H:%M:%S'), winner if winner else '', is_draw, first_move, first_player_result])
